...
public class ExplodeandmotionProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, double intensity, double power) {
			...
			if (world instanceof ServerLevel _level)
				_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
						("/summon falling_block ~ ~ ~ {BlockState:{Name:\"" + ""
								...
								+ "\"},Time:1,Motion:[" + Mth.nextDouble(RandomSource.create(), power / 4 + intensity * (-2), intensity) + "d," + (power / 4 + Mth.nextDouble(RandomSource.create(), 1, 2)) + "d,"
								+ Mth.nextDouble(RandomSource.create(), power / 4 + intensity * (-2), intensity) + "d]}"));
		}
